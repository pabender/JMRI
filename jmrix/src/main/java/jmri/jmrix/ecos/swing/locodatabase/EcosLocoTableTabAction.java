...
import jmri.beantable.AbstractTableTabAction;
...
public class EcosLocoTableTabAction extends AbstractTableTabAction<NamedBean> {  // there is no specific subtype of NamedBean here, see EcosLocoAddressManager
    ...
    @Override
    public void addToFrame(jmri.beantable.BeanTableFrame<NamedBean> f) {
        if (tabbedTableArray.size() > 1) {
            super.addToFrame(f);
        }
    }
    ...
    @Override
    public void setMenuBar(jmri.beantable.BeanTableFrame<NamedBean> f) {
        if (tabbedTableArray.size() > 1) {
            super.setMenuBar(f);
        }
    }
